...
public class Code07_CutOrPoison {
    ...
    public static boolean f(int[] cuts, int[] poisons,long hp,int round){
        int n = Math.min(cuts.length,round) ;
        for(int i=0;i<n;i++){
            hp -= Math.max((long)cuts[i], (long) poisons[i] *(round-i-1));
            if(hp<=0) return true;
        }
        return hp<=0;
...
这道题确实是体现了二分答案法出现的原因，正着想很难想，所以我们看到`答案有范围`->`有单调性`->`构建f函数去二分答案`
