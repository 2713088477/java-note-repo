...
这道题的dp[i]的含义就是以"i"结尾的最长有效括号的长度

这道题的转移方程有点东西

> 1.如果i字符为'(',dp[i]=0
>
> 2.如果i字符为')',要去找e(e=i-dp[i-1]-1),如果e所在字符为'(',这样就和i字符匹配上了
>
> ​	同时如果dp[e-1]是合法的话，最后dp[i] = dp[i] = 2 + dp[i-1] + (e-1 >= 0 ? dp[e-1] : 0);
...
这道题比较特殊，不是之前的以"i"结尾统计答案数

这道题是dp[x]表示以x结尾的最长的字串，就是以最长的字串来去重

比如`xabca` 其中`a`的最长字串是`xa`，统计最长的，自然达到了去重的效果

 然后如何统计以某个字符结尾最长的字串，就遍历一遍就行了