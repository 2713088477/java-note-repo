...
一次命令的响应时间=1次往返的网路传输耗时+1次Redis执行命令耗时

![](assets/单个命令执行流程.png)

`往往一次往返的网络传输耗时 > 一次Redis执行命令耗时`
...
<font color="red">不要在一次批处理种传输太多命令，否则单次命令占用带宽过多，会导致网络阻塞</font>
...
如MSET或Pipeline这样的批处理需要在一次请求种携带多条命令，而此时如果Redis是一个集群，那批处理命令的多个key必须落在一个插槽中，否则就会导致执行失败。

|          | 串行命令             | 串行slot                                                         | 并行slot                                                         | hash_tag                            |
| -------- | ---------------- | -------------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------- |
| **实现思路** | for循环遍历，依次执行每个命令 | 在客户端计算每个key的slot，将slot一致分为一组，每组都利用Pipeline批处理。<br>**串行执行各组命令** | 在客户端计算每个key的slot，将slot一致分为一组，每组都利用Pipeline批处理。<br>**并行执行各组命令** | 将所有key设置相同的hash_tag，则所有key的slot一定相同 |
...
Spring会整合集群的批处理，对于程序员几乎是无感知的

### 6.服务端优化

**持久化配置**

Redis 的持久化虽然可以保证数据安全，但也会带来很多额外的开销。因此，请遵循以下建议：

> 1.缓存场景禁用持久化
> 用来做缓存的 Redis 实例尽量不要开启持久化功能。
> 2.优先使用 AOF，关闭 RDB
> 建议关闭 RDB 持久化功能，使用 AOF 持久化（更细粒度、更安全）。
> 3.在 Slave 节点定期备份 RDB
> 利用脚本定期在 slave 节点执行 BGSAVE 或手动触发 RDB，实现数据备份，避免影响主节点性能。
> 4.合理设置 AOF rewrite 阈值
> 设置合理的 auto-aof-rewrite-percentage 和 auto-aof-rewrite-min-size，避免频繁的 bgrewriteaof 操作。
> 5.禁止 rewrite 期间同步 AOF
> 配置 no-appendfsync-on-rewrite = yes，禁止在 AOF 重写期间执行 fsync，避免因磁盘 I/O 阻塞导致服务延迟。



**部署相关建议**

> 1.预留足够内存应对 fork & rewrite
> Redis 实例所在的物理机要预留足够内存，以应对 fork() 子进程和 AOF/RDB 重写时的内存复制开销。
> 2.控制单实例内存上限
> 单个 Redis 实例内存上限不要太大（如 4GB 或 8GB），可加快 fork 速度、减少主从同步压力、降低数据迁移成本。
> 3.避免与 CPU 密集型应用混部
> 不要将 Redis 与 CPU 密集型应用部署在同一台机器上，防止资源争抢影响响应延迟。
> 4.避免与高硬盘负载应用混部
> 不要与数据库、消息队列等高磁盘 I/O 负载的应用部署在一起，避免磁盘竞争导致持久化阻塞或性能下降。
...
Redis会绑定在0.0.0.0:6379,这样将会将Redis服务暴露到公网上，而Redis如果没有做身份认证，会出现严重的安全漏洞，漏洞重现方式:

[Redis未授权访问配合SSH key文件利用分析-腾讯云开发者社区-腾讯云](https://cloud.tencent.com/developer/article/1039000)
...
当Redis内存不足时，可能导致key频繁被删除、响应时间变长、QPS不稳定。当内存使用率达到90%以上时就需要我们警惕，并快速定位到内存占用原因。
...
- **AOF缓冲区:** AOF刷盘之前的缓存区域，AOF执行rewirite的缓冲区。无法设置容量上限
...
<font color="red">单体Redis(主从Redis)已经能达到万级别的QPS,并且也具备很强的高可用特性。如果主动能满足业务需求的情况下，尽量不搭建Redis集群</font>
