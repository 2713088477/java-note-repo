...
GEO是Geolocation的简写形式，代表地理坐标。Redis在3.2版本中加入了对GEO的支持，允许存储地理坐标信息，帮我我们根据经纬度来检索数据。常见的命名有:

1. **GEOADD** :添加一个地理空间信息，包含:经度(longitude)、维度(latitude)、值(member)

2. **GEODIST** :计算指定的两个点之间的距离并返回

3. **GEOHASH** :将指定member坐标转为hash字符串的形式并返回

4. **GEOPOS** :返回指定member的坐标

5. **GEORADIUS** :指定圆心，半径，找到改圆内包含的所有member,并按照与圆心之间的距离排序后并返回。6.2以后已废弃
    ...
    public Result queryShopByType(Integer typeId, Integer current, Double x, Double y) {
        ...
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo()
                .search(key,
                        ...
                        RedisGeoCommands.GeoSearchCommandArgs.newGeoSearchArgs().includeDistance().limit(Long.valueOf(end)));
        //4.非空判断
        if(results==null){
            return Result.ok(Collections.emptyList());
        }
...
**Redis中**是利用string类型数据结构实现BitMap,因此最大上限是512M,转换为bit则是2^32个bit位

**常见命令:**

| 命令                                      | 功能                   | 时间复杂度 | 示例                               |
| --------------------------------------- | -------------------- | ----- | -------------------------------- |
| `SETBIT key offset value`               | 设置指定偏移量的位值（0/1）      | O(1)  | `SETBIT user:1001:sign 0 1`      |
| `GETBIT key offset`                     | 获取指定偏移量的位值           | O(1)  | `GETBIT user:1001:sign 0`        |
| `BITCOUNT key [start end]`              | 统计 1 的个数             | O(N)  | `BITCOUNT user:1001:sign`        |
| `BITPOS key bit [start end]`            | 查找第一个 0 或 1 的位置      | O(N)  | `BITPOS user:1001:sign 1`        |
| `BITOP operation destkey key [key ...]` | 位运算（AND/OR/XOR/NOT）  | O(N)  | `BITOP AND result key1 key2`     |
| `BITFIELD key [GET type offset]`        | 批量操作多个位域             | O(1)  | `BITFIELD user:1001 GET u8 0`    |
| `BITFIELD_RO`                           | BITFIELD 的只读版本（6.2+） | O(1)  | `BITFIELD_RO user:1001 GET u8 0` |
...
```java
    ...
    public Result signCount() {
        ...
        //5.从Redis中取出签到数据 bitfield key get u[dayOfMonth] 0
...
Redis中的HLL是基于string结构实现的，**单个HLL的内存永远小于16kb**，内存占用低的令人发指！作为代价，其测量结果是概率性的，有**小于0.81%的误差**。不过对于UV统计来说，这完全可以忽略。
...
```java
    ...
    public void testHLL(){
        ...
        for(int i=0;i<1000000;i++){
            j = i % 1000;
            users[j] = "user:" + i;
            if(j == 999) stringRedisTemplate.opsForHyperLogLog().add("hll1",users);
        }
